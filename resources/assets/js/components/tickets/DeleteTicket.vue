            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                       <p> Vous voulez vraiment supprimer cet Ticket .</p>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-default" @click="hideCard">Retour</button>
                        <button type="button" class="btn btn-primary" @click="deleteticket">Supprimer</button>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    export default {
        ...
        methods: {
            ...
            deleteticket() {
                this.$parent.loading_id = this.id_ticket;
                axios.delete('/gestiontickets/'+this.id_ticket).then((response)=>{
                    if(response.data.delete) {
                        this.$parent.loading_id =  -1;
                        this.hideCard();
                        this.$parent.getTickets();
                        this.$parent.messageAlert ="Ticket à été supprimer avec success ...";
