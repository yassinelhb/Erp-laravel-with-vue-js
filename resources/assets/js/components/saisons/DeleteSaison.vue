            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                       <p> Vous voulez vraiment supprimer cet Saison . </p>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-default" @click="hideCard">Retour</button>
                        <button type="button" class="btn btn-primary" @click="deletesaison">Supprimer</button>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    export default {
        ...
        methods: {
            ...
            deletesaison() {
                this.$parent.loading_id = this.id_saison;
                axios.delete('/gestionsaisons/'+this.id_saison).then((response)=>{
                    if(response.data.delete) {
                        this.$parent.loading_id =  -1;
                        this.hideCard();
                        this.$parent.getSaisons();
                        this.$parent.messageAlert ="Saison à été supprimer avec success ...";
